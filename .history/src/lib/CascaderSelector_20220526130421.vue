  ...
  <div class="block">
    ...
    <span>
      ...
      <div class="cas" v-if="actId>-1 && source[actId].children">
        <IniCascader  v-model:source="source[actId].children" :activeId="activeId2" @patch="onPatch" :itemGroud="copyItem">
        </IniCascader>
      </div>
  </div>
</template>
...
export default {
  ...
  setup(props, content) {
    ...
    console.log(props.itemGroud, '传递来的数组')
    ...
    const choose = (index, event) => {
        ...
        copyItem = copyItem.concat(props.itemGroud)
        copyItem.values.push(props.source[actId.value].name)
    ...
    return{
      choose, activeId2, onPatch, actId, copyItem
    }
  },
};
</script>
