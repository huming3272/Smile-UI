<template>
  <div class="block">
    ...
    <span>
      ...
      <div v-if="actId>-1 && source[actId].children">
        <CascaderSelector class="cas" v-model:source="source[actId].children" :activeId="activeId2" v-on:patch="onPatch">
        </CascaderSelector>
      </div>
  </div>
</template>
...
export default {
  ...
  setup(props, content) {
    ...
    const choose = (index, event) => {
        activeId2.value = -1;
        actId.value = index
        console.log(actId.value, 'choose')
