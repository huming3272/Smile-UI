...
import { computed,
    ...
    export default {
        ...
        setup(props, context){
            const navItems = ref < HTMLDivElement[] >([]).value
            const indicator = ref < HTMLDivElement >(null).value
            const container = ref < HTMLDivElement >(null).value
            const x = () => {
              const divs = navItems
              ...
              const { left: left2 } = indicator.getBoundingClientRect()
