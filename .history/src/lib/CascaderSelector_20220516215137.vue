  ...
  <div class="block">
    ...
    <span>
      ...
      <div class="cas" v-if="actId>-1 && source[actId].children">
        <CascaderSelector  v-model:source="source[actId].children" :activeId="activeId2" v-on:patch="onPatch">
        </CascaderSelector>
      </div>
  </div>
</template>
...
export default {
  ...
  setup(props, content) {
    console.log(props.source[0].value)
    let activeId2 = ref(0)
    const actId = ref(0)
    const choose = (index, event) => {
        activeId2.value = -1;
        actId.value = index
        console.log(props.source[actId.value].children.value, 'choose')
        // console.log(props.source[actId].children)
        if (!props?.source[actId]?.children) {
          content.emit("patch", props.source[actId].name)
        }
        // console.dir(event)
    }
    const onPatch = (val) => {
